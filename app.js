...
const tcpipLayers=[
 ...
 [3,'Transport','TCP, UDP','#34a853','Segment / Datagram','Transporta datos entre procesos y utiliza puertos.'],
 [2,'Internet','IP, ICMP','#f9ab00','Packet','Realiza direccionamiento lógico y encaminamiento entre redes.'],
 [1,'Network Access','Ethernet, Wi-Fi, ARP','#ff6d00','Frame / Bits','Agrupa las funciones de Data Link y Physical.']
];
...
function getLearnData(){
  ...
  return originalLayers.map(x=>{
    const extra={
      7:['Data','This is where las aplicaciones utilizan servicios de red y generan información para comunicar.'],
      6:['Data','It covers la representación, codificación, cifrado y compresión de los datos.'],
      5:['Data','Covers session establishment, maintenance, and termination.'],
      4:['Segment / Datagram','Transporta información entre procesos through puertos y protocolos como TCP y UDP.'],
      3:['Packet','Adds direccionamiento IP y permite el encaminamiento entre diferentes redes.'],
      2:['Frame','Construye tramas para el enlace local y utiliza direcciones MAC y tecnologías como Ethernet.'],
...
function renderLearn(){
  const protocols={
    ethernet:{title:'Ethernet II',layer:'2 — Data Link',pdu:'Frame',color:'#ff6d00',intro:'Ethernet II encapsula a packet de Capa 3 dentro de una trama para un enlace local. El header Ethernet II tiene 14 bytes; el FCS tiene 4 bytes y se considera aparte.',fields:[
      ...
      ['EtherType',16,2,'0x0800','Indicatestes the encapsulated protocol; 0x0800 identifies IPv4.','NIC / driver','NIC / Layer 2'],
    ...
    ipv4:{title:'IPv4',layer:'3 — Network',pdu:'Packet',color:'#f9ab00',intro:'IPv4 provides logical addressing and routing. The base header is 20 bytes and can grow when options are present.',fields:[
      ['Version',4,0.5,'0100 / 4','Indicatestes the IP version: IPv4.','Host / router','Host / router'],
      ['IHL',4,0.5,'0101 / 5','Indicates the length dthe header en palabras de 32 bits. IHL=5 significa 20 bytes.','Host / router','Host / router'],
      ...
      ['Fragment Offset',13,1.625,'0','Indicatestes the fragment position within the original payload, in 8-byte units.','Host / router','Destination'],
    ...
    tcp:{title:'TCP',layer:'4 — Transport',pdu:'Segment',color:'#34a853',intro:'TCP provides connection-oriented transport, sequencing, ACKs, flow control, and retransmission. Its minimum header is 20 bytes.',fields:[
      ...
      ['Acknowledgment Number',32,4,'2001','Indicatestes the next byte expected by the receiver when ACK is set.','Source host','Destination host'],
      ['Data Offset',4,0.5,'0101 / 5','Indicates the length dthe header en palabras de 32 bits.','Source host','Destination host'],
    ...
    icmp:{title:'ICMP Echo',layer:'3 — Network',pdu:'Mensaje ICMP',color:'#f9ab00',intro:'ICMP communicates IP control and diagnostic information. Ping uses Echo Request and Echo Reply.',fields:[
    ...
    arp:{title:'ARP',layer:'2 — Data Link / soporte de Capa 3',pdu:'Mensaje ARP',color:'#ff6d00',intro:'ARP resolves an IPv4 address to a MAC address on the local LAN. The request is usually broadcast; the reply is usually unicast.',fields:[
      ...
      ['OPER',16,2,'1 request / 2 reply','Indicatestes an ARP request or reply.','Host','Host'],
  ...
  function renderOverview(){
    panel.innerHTML=`<div class="deepIntroGrid">
      ...
      <div class="deepIntroCard"><h2>Regla mental</h2><div class="formulaFlow"><span>Data</span><b>+</b><span>TCP/UDP</span><b>+</b><span>IPv4</span><b>+</b><span>Ethernet</span><b>→</b><span>Bits</span></div><p>En el destino the process se invierte: bits → trama → paquete → segmento/datagrama → datos.</p></div>
    </div>
    <div class="deepSection"><h2>PDUs and responsibilities</h2><div class="pduGrid"><div><b>Application</b><span>Data</span></div><div><b>Transport</b><span>Segment TCP / Datagram UDP</span></div><div><b>Network</b><span>Packet IPv4 / IPv6</span></div><div><b>Enlace</b><span>Ethernet frame</span></div><div><b>Physical</b><span>Bits / señales</span></div></div></div>
  ...
  function renderEncap(){
    panel.innerHTML=`<div class="deepSection"><h2>Encapsulation — step by step</h2><p>See what each layer adds. The example sizes are deliberately small so the arithmetic is easy to follow.</p><div class="encapFlow">
      ...
      <div class="encapStep tcpStep"><b>2 · TCP SEGMENT</b><span>TCP header 20 B + datos 85 B</span><strong>105 B</strong><small>Transport adds puertos, secuencia, ACK, flags, etc.</small></div><div class="encapArrow">↓</div>
      <div class="encapStep ipStep"><b>3 · IPv4 PACKET</b><span>IPv4 header 20 B + segmento 105 B</span><strong>125 B</strong><small>Network adds Source IP/destino, TTL, protocolo, etc.</small></div><div class="encapArrow">↓</div>
...
function renderInspect(){
  const fields=[
    ['PREAMBLE','8 B','#777',8,'Sincroniza la recepción antes de la transmisión.'],
    ['DEST MAC','6 B','#1a73a8',6,'Identifies el destinatario dentro del enlace local.'],
    ['SRC MAC','6 B','#34a853',6,'Identifies the interface transmitting the frame.'],
    ['TYPE','2 B','#f9ab00',2,'Indicates el protocolo transportado; por ejemplo 0x0800 para IPv4.'],
  ...
  const ieee=[
    ...
    ['802.11','Wi‑Fi','Familia de estándares para redes LAN inalámbricas. Utiliza el medio radioeléctrico y mecanismos específicos para el acceso inalámbrico.','Data Link / Physical','Networkes WLAN'],
    ['802.1Q','VLAN','Estándar utilizado para identificar VLAN through etiquetado dentro de tramas Ethernet, permitiendo segmentación lógica de redes.','Data Link','Etiquetado VLAN'],
  ...
  <div class="inspectGrid"><div class="inspectBox"><h3>Ethernet frame</h3><div class="inspectFields" id="inspectFields"></div><p id="inspectText">Select a field.</p></div><div class="inspectBox"><h3>Minimum length</h3><p>Desde Destination MAC hasta FCS, una trama Ethernet mínima tiene <b>64 bytes</b>: 14 bytes de header + minimum 46 bytes de payload + 4 bytes de FCS.</p></div></div>
    ...
    panel.innerHTML=`
      ...
      <div class="manchesterControls">
        <div>
          ...
          <small>Usa únicamente 0 y 1. Maximum 16 bits.</small>
  ...
  function render4B5BPanel(){
    ...
    panel.innerHTML=`
      <div class="codingIntro">
        ...
        <div class="conventionBox"><b>Important:</b> 4B/5B no dibuja directamente una forma de onda como Manchester. Primero transforma los datos. Después, el resultado puede pasar a un método de señalización de la layer física.</div>
      </div>
      ...
      <div class="fourBControls">
        <div>
          ...
          <small>Usa una cantidad de bits múltiplo de 4. Maximum 32 bits.</small>
          <div class="manchesterActions">
...
function renderNetworkLab(){
  ...
  function labARP(){
    const steps=[
      ['1. PC A necesita una MAC','The host wants to send traffic to gateway 192.168.1.1, but initially knows only its IP address.'],
      ...
      ['5. Tabla ARP actualizada','PC A temporarily stores the IP ↔ MAC mapping and can now build the Ethernet frame.']
    ];
    ...
    <div id="arpStep" class="processCard"></div><div class="miniTable"><b>Tabla ARP de PC A</b><div id="arpTable">192.168.1.1 → <em>unknown</em></div></div>`;
  ...
  function labDNS(){
    const steps=[
      ...
      ['Conexión','With an IP address available, the client can continue communication toward the destination.']
    ];
    ...
    <div class="labControls"><button id="dnsPrev">◀ Paso</button><button id="dnsNext">Step ▶</button></div><div id="dnsStep" class="processCard"></div>`;
  ...
  function labNAT(){
    ...
    <div class="natDiagram"><div class="natSide"><b>RED PRIVADA</b><div>PC A<br><small>192.168.1.10:51500</small></div><div>PC B<br><small>192.168.1.20:51501</small></div></div><div class="natDevice"><b>ROUTER NAT</b><small>192.168.1.1<br>203.0.113.10</small><button id="natTranslate">Translate</button></div><div class="natSide"><b>INTERNET</b><div>Servidor<br><small>Destination: 142.250.x.x:443</small></div></div></div>
    ...
    natTranslate.onclick=()=>natResult.innerHTML='<b>After NAT/PAT:</b><p>203.0.113.10:40001 → servidor:443</p><small>The NAT device maintains a table that associates the translated flow with the corresponding internal host.</small>';
  }
  ...
  function labVLAN(){
    ...
    <div id="vlanVisual" class="vlanVisual"></div><div class="tagBox">Example de trama etiquetada: <b>MAC Destination | MAC Source | 802.1Q: VLAN <span id="vlanId">10</span> | EtherType | Data</b></div>`;
  ...
  function labSTP(){
    ...
    <div class="labControls"><button id="stpToggle">Show / hide logical blocking</button></div><div id="stpText" class="processCard"><b>Networkundant topology:</b><p>SW1 actúa como referencia conceptual de Root Bridge. The SW2–SW3 link is shown blocked in this simplified model to prevent an active redundant path that could create a loop.</p></div>`;
  ...
  function labRouting(){
    ...
    <div class="routeTable" id="routeTable"></div><div class="labControls"><button id="routePrev">◀ Salto</button><button id="routeNext">Next salto ▶</button></div><div id="routeStep" class="processCard"></div>`;
  ...
  function labSubnet(){
    panel.innerHTML=`<div class="labIntro"><h2>Subnetting — Visually divide an IPv4 network</h2><p>Enter a network address and prefix. The lab calculates the mask, number of addresses, and block range. This tool works with IPv4 networks and shows basic prefix and block-size concepts.</p></div>
    <div class="subnetControls"><label>IP / Network<input id="subIp" value="192.168.1.0"></label><label>Prefix<select id="subPrefix">${Array.from({length:31},(_,i)=>`<option value="${i+1}" ${i+1===24?'selected':''}>/${i+1}</option>`).join('')}</select></label><button id="subCalc">Calculateste</button></div>
    ...
    function calc(){
      try{
        ...
        const bits=32-p;
        subExplain.innerHTML=`<b>How it is calculated:</b><p>A prefix /${p} reserves ${p} bits for the network portion y leaves ${bits} bits for the host portion. Therefore the block contains 2<sup>${bits}</sup> = <b>${size.toLocaleString()}</b> addresses. Para prefijos tradicionales de /30 o menores, el cálculo habitual de hosts utilizables es 2<sup>${bits}</sup> − 2.</p><small>* /31 y /32 are used in special scenarios and do not follow this simplified usable-host calculation.</small>`;
